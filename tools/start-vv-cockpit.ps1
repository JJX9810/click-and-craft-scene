<#
  ...
  ------------------------------------------------------------
  - prueft/startet Ollama (lokal), optional CPU-Modus
  - startet lokalen HTTP-Server (Python) fuer das Cockpit
  - oeffnet das Cockpit im Browser ueber http://localhost  (KEIN file://)
  - schreibt nexus-local-config.json (informativ, optional vom Cockpit ladbar)
  Aufruf: ueber start-vv-cockpit.bat (Doppelklick)
  Hinweis: Datei bewusst in reinem ASCII gehalten (Windows PowerShell 5.1).
  ============================================================
#>
...
# Projektwurzel = uebergeordneter Ordner von tools\
...
if (-not $foundRel) {
  ...
  Err "Cockpit-Datei fehlt. Bitte vvcockpit_26.html unter cockpit\vvcockpit_26.html ablegen."
  exit 1
}
$CockpitFile = $foundRel.Replace('\','/')
Ok "Cockpit-Datei gefunden: $CockpitFile"

# ---------------- 2) Freien Cockpit-Port waehlen ----------------
$port = $CockpitPort
if (-not (Test-PortFree $port)) {
  Warn "Port $port ist belegt. Suche freien Port ..."
  $found = $false
  for ($p = $CockpitPort + 1; $p -le $CockpitPort + 20; $p++) {
...
# Erlaubte Origins inkl. tatsaechlich genutztem Port
$origins = @("http://localhost:$port","http://127.0.0.1:$port") + $AllowOrigins | Select-Object -Unique

# ---------------- 3) Ollama pruefen / starten ----------------
Info "Pruefe Ollama ..."
$ollamaCmd   = Get-Command ollama -ErrorAction SilentlyContinue
$ollamaApiUp = Test-HttpOk "$OllamaBaseUrl/api/tags"

if ($ollamaApiUp) {
  Ok "Ollama API erreichbar ($OllamaBaseUrl) - laeuft bereits."
  if ($ForceOllamaCpu) {
    Warn "Ollama laeuft bereits. Falls CUDA-Fehler auftreten, Ollama komplett beenden und den Starter erneut oeffnen."
    if ($RestartOllamaIfCudaModeRequired) {
      Warn "RestartOllamaIfCudaModeRequired=true gesetzt - wird aus Sicherheitsgruenden NICHT automatisch ausgefuehrt."
    }
  }
}
elseif ($ollamaCmd) {
  ...
  for ($i = 0; $i -lt 10; $i++) {
    ...
    if (Test-HttpOk "$OllamaBaseUrl/api/tags") { $up = $true; break }
  }
  ...
  Warn "Ollama fehlt. Bitte Ollama installieren: https://ollama.com"
  Warn "Das Cockpit startet trotzdem. NEXUS nutzt dann nur die lokale Befehlslogik."
}

# ---------------- 4) Ollama-Modell pruefen (kein Auto-Download) ----------------
if ($ollamaApiUp) {
  try {
    $tags = Invoke-RestMethod -Uri "$OllamaBaseUrl/api/tags" -TimeoutSec 5
    $models = @()
    if ($tags.models) { $models = @($tags.models | ForEach-Object { $_.name }) }
    $base = ($OllamaModel -split ":")[0]
    $has  = $models | Where-Object { $_ -eq $OllamaModel -or (($_ -split ":")[0] -eq $base) }
    if ($has) {
      Ok "Modell gefunden: $OllamaModel"
    }
    else {
      Warn "Modell '$OllamaModel' wurde nicht gefunden."
      if ($models.Count) { Info ("Vorhandene Modelle: " + ($models -join ", ")) }
      else { Info "Aktuell sind keine Modelle installiert." }
      Warn "Im Cockpit ein vorhandenes Modell waehlen ODER im Terminal laden: ollama pull $base"
...
if ($EnableGeminiProxy) {
  ...
  if (-not $hasKey -and (Test-Path -LiteralPath $envFile)) {
    ...
    if ($line) {
      ...
      if ($v -and $v -notmatch $placeholder) { $hasKey = $true }
    }
  }

  if (Test-HttpOk "http://localhost:$GeminiProxyPort/health") {
    Ok "Gemini-Proxy laeuft bereits (Port $GeminiProxyPort)."
    $geminiUp = $true
  }
  elseif (-not (Test-Path -LiteralPath $proxyJs)) {
    Warn "Gemini-Proxy-Datei nicht gefunden - uebersprungen. (Cockpit nutzt Ollama/lokal.)"
  }
  elseif (-not $node) {
    Warn "Node.js fehlt. Bitte Node.js installieren: https://nodejs.org - Gemini-Proxy uebersprungen, Ollama/lokal bleibt aktiv."
  }
  elseif (-not $hasKey) {
    Warn "Gemini-Key nicht gefunden. Gemini-Fallback deaktiviert. Ollama/lokaler Fallback bleibt aktiv."
  }
  else {
    ...
    for ($i = 0; $i -lt 8; $i++) {
      ...
      if (Test-HttpOk "http://localhost:$GeminiProxyPort/health") { $geminiUp = $true; break }
    }
    if ($geminiUp) { Ok "Gemini-Proxy erreichbar (http://localhost:$GeminiProxyPort)." }
    else { Warn "Gemini-Proxy reagiert noch nicht - Cockpit startet trotzdem." }
  }
}
...
if ($pc) { $pyExe = $pc.Source }
elseif ($pyc) { $pyExe = $pyc.Source }
if (-not $pyExe) {
  Err "Python fehlt. Bitte Python installieren und Add to PATH aktivieren: https://www.python.org/downloads/"
...
for ($i = 0; $i -lt 15; $i++) {
  ...
  if (Test-HttpOk "http://localhost:$port/") { $ready = $true; break }
}
if ($ready) { Ok "Cockpit-Server laeuft auf Port $port." }
else { Warn "Server antwortet noch nicht ganz - Browser wird trotzdem geoeffnet." }
...
try {
  $cfg = [ordered]@{
    ...
    geminiProxyUrl = ("http://localhost:$GeminiProxyPort")
    geminiEnabled  = $geminiUp
  }
...
# ---------------- 7) Browser oeffnen ----------------
if ($OpenBrowser) {
  Info "Oeffne Browser: $cockpitUrl"
  $opened = $false
...
Write-Host ""
Ok "FERTIG. Cockpit laeuft: $cockpitUrl"
Info "- Dieses Fenster OFFEN lassen, solange du arbeitest."
Info "- Mikrofon und NEXUS funktionieren hier (localhost = sicherer Kontext)."
Info "- Zum Beenden: dieses Fenster schliessen (oder stop-vv-cockpit.bat)."
Write-Host ""

# Blockiert, bis der Server-Prozess endet (Fenster schliessen beendet ihn).
